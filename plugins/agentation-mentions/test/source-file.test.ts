...
test("a served bundle path is not a source location", () => {
  ...
  assert.equal(usableSourceFile("api/v1/plugins/agentation-mentions/assets/app.js:10771:17"), false);
...
test("a bundle source is dropped rather than sent to the agent", () => {
  const cleaned = withoutBundleSource(
    annotation("api/v1/plugins/agentation-mentions/assets/app.js:10771:17"),
  );
