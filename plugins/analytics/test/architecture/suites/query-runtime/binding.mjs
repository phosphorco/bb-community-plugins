    ...
    async create({ trustedSource, resourceLimits, testControl } = {}) {
      ...
      const runtime = await production.createQueryRuntime({
        observer: (event) => {
          ...
          if (events.length >= maxObserverEvents || bytes > maxObserverBytes || capturedEventBytes + bytes > maxObserverBytes) {
...
export function isAllowedObserverEvent(event) {
  ...
  if (event.kind === "completion" && event.outcome === "error" && event.resultBytes !== 0) return false;
