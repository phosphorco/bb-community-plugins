...
function assertQueueEvidence(events) {
  ...
  if (!boundedCounts || cacheEvents.length === 0 || [...dispatches, ...cacheEvents].some((event) => !isAllowedObserverEvent(event)))
    return { id: "queue-and-cache-observability", status: "fail", details: "Dispatch/cache observations lack complete bounded closed-schema envelopes or cache evidence." };
...
async function runInstrumentSelfTest() {
  ...
  const observerEnvelopeControls = selfObserverEnvelopeControls();
  ...
  const checks = [
    ...
    { id: "self-test-observer-exact-envelope-controls", status: observerEnvelopeControls.pass ? "pass" : "fail", details: observerEnvelopeControls.pass ? "Every closed observer envelope accepts only its complete own-key shape; skeletal source/cache, missing fields, invalid completion variants, and skeletal cache queue evidence fail." : "Observer exact-envelope assertion controls failed." },
...
function selfObserverEnvelopeControls() {
  const hash = "a".repeat(64);
  const executionId = "analytics-exec_selftest_observer_12345678";
  const physicalKey = "analytics-physical_selftest_observer_12345678";
  const events = [
    { kind: "child-spawn", monotonicMs: 1, pid: 7, entrySha256: hash },
    { kind: "bootstrap-ready", monotonicMs: 2, pid: 7, bootstrapFingerprint: hash },
    { kind: "source-materialized", monotonicMs: 3, pid: 7, executionId, materializationId: hash, childReadCount: 2, reused: false },
    { kind: "dispatch", monotonicMs: 4, pid: 7, executionId, physicalKey, queued: 0, queuedBytes: 0, sqlExecutionCount: 1 },
    { kind: "completion", monotonicMs: 5, executionId, outcome: "success", resultBytes: 12 },
    { kind: "completion", monotonicMs: 6, executionId, outcome: "error", code: "result-limit", resultBytes: 0 },
    { kind: "cache", monotonicMs: 7, physicalKey, hit: true, entries: 1, bytes: 12 },
    { kind: "kill-requested", monotonicMs: 8, executionId, pid: 7, reason: "deadline" },
    { kind: "child-exit", monotonicMs: 9, executionId, pid: 7, code: null, signal: "SIGKILL" },
    { kind: "close-confirmed", monotonicMs: 10, pid: 7, queued: 0, entries: 0, bytes: 0 },
  ];
  const everyComplete = events.every((event) => isAllowedObserverEvent(event));
  const everyMissingFieldRejected = events.every((event) => Object.keys(event).every((field) => {
    const missing = structuredClone(event);
    delete missing[field];
    return !isAllowedObserverEvent(missing);
  }));
  const skeletalSourceRejected = !isAllowedObserverEvent({ kind: "source-materialized", monotonicMs: 1 });
  const skeletalCacheRejected = !isAllowedObserverEvent({ kind: "cache", monotonicMs: 1 });
  const successWithCodeRejected = !isAllowedObserverEvent({ ...events[4], code: "result-limit" });
  const { code: omittedCode, ...errorWithoutCode } = events[5];
  const errorWithoutCodeRejected = omittedCode === "result-limit" && !isAllowedObserverEvent(errorWithoutCode);
  const errorWithResultRejected = !isAllowedObserverEvent({ ...events[5], resultBytes: 1 });
  const skeletalCacheQueueRejected = assertQueueEvidence([events[3], { kind: "cache", monotonicMs: 11 }]).status === "fail";
  const completeQueueAccepted = assertQueueEvidence([events[3], events[6]]).status === "pass";
  return {
    pass: everyComplete && everyMissingFieldRejected && skeletalSourceRejected && skeletalCacheRejected
      && successWithCodeRejected && errorWithoutCodeRejected && errorWithResultRejected && skeletalCacheQueueRejected && completeQueueAccepted,
  };
}
