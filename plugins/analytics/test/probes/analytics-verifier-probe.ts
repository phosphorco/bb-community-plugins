// Offline-only semantic/binder witness. Never import this from production.
...
import { createProbeQueryRuntime } from "./legacy-query-runtime.mjs";
...
/**
 * Live bundle verification is unavailable until the qualified execution
 * service exists. It must not obtain a DB-path probe runtime as a fallback.
 */
...
/** Offline-only semantic/binder witness for CI; never call from server/RPC. */
export async function verifyAnalyticsBundleOfflineProbe(
  bundle: AnalyticsBundle,
): Promise<AnalyticsBundleVerification> {
  const fixture = await createVerifierFixture();
  let runtime: Awaited<ReturnType<typeof createProbeQueryRuntime>> | undefined;
  let failed = false;
  try {
    runtime = await createProbeQueryRuntime({ trustedSource: fixture.handoff });
