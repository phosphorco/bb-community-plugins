// Offline-only legacy DB-path runtime witness. Never import from production code.
...
const CHILD_ENTRY = fileURLToPath(new URL("./legacy-query-runtime-worker.cjs", import.meta.url));
...
const SAFE_MESSAGES = Object.freeze({
  ...
  "isolation-unavailable": "Analytics execution isolation is not available.",
 ...
 */
/**
 * Legacy DB-path runtime is hard-denied in ordinary code. It cannot satisfy
 * the immutable-snapshot/OS-isolation contract and must never become a live
 * fallback while Analytics is disabled.
 */
export async function createQueryRuntime() {
  throw new RuntimeFault("isolation-unavailable");
}

/** Explicit test/probe boundary for retained architecture witnesses only. */
export async function createProbeQueryRuntime({
  trustedSource,
  resourceLimits,
  observer,
  testControl = {},
