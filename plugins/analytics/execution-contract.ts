...
export const storedExecutionRecordSchema = z
  ...
  .superRefine((record, context) => {
    ...
    if (record.result.executionId !== record.result.resolved.executionId)
      context.addIssue({
        code: "custom",
    ...
    if (
      record.definition.bundle.id !== record.result.resolved.bundleId ||
      record.definition.bundle.revision !==
        record.result.resolved.bundleRevision ||
      canonicalJson(record.definition.query) !==
        canonicalJson(record.result.resolved.query)
    )
      context.addIssue({
        code: "custom",
        message:
          "Stored definition must exactly match captured execution bundle/query authority.",
      });
    const fields = new Set(
      record.result.result.columns.map((column) => column.name),
    );
    for (const figure of record.definition.figures) {
      for (const name of visualizationFields(figure.visualization))
        if (!fields.has(name))
          context.addIssue({
            code: "custom",
            message:
              "Visualization field " + name + " is absent from result schema.",
          });
      if (
        canonicalJson(figure.plotted.total) !==
          canonicalJson(record.result.result.resultExtent) ||
        figure.plotted.plottedRows > record.result.result.rows.length ||
