...
import { skillContributor, skillsDashboardData, skillsQueryFilterFromDashboard } from "../analytics-model.ts";
import type { SkillQueryFilter } from "../skill-query-schema.ts";
import { SkillsDashboard } from "../skills-dashboard/skills-dashboard.tsx";
import type { SkillsDashboardFilters } from "../skills-dashboard/skills-dashboard.tsx";
import type { BrowserDashboardResult, BrowserQueryResult } from "../browser-engine.ts";
import type { AnalyticsBundleResponse, AnalyticsCatalogResponse, SkillsQueryResponse, rpcContract } from "../rpc-contract.ts";
...
// Browser DuckDB and ECharts are optional execution/rendering kernels. The
// metadata-first plugin entry stays declarative until the Analytics route and
// then the relevant dashboard/card are actually opened. Artifact-format-v2
// turns these dynamic imports into integrity-bound chunks instead of folding
// them back into app.js.
async function getBrowserAnalyticsEngineOnDemand() {
  return (await import("../browser-engine.ts")).getBrowserAnalyticsEngine();
}

const EChartsFigure = lazy(async () => {
  const module = await import("../echarts-figure.tsx");
  return { default: module.EChartsFigure };
});

class EChartsChunkBoundary extends Component<Readonly<{ children: ReactNode }>, Readonly<{ failed: boolean }>> {
  state = { failed: false };
  ...
  render(): ReactNode {
    if (this.state.failed) {
      return <p className="analytics-echart-loading" role="status">
        The chart renderer could not load. Exact values remain available below. <button type="button" onClick={() => window.location.reload()}>Reload page</button>
      </p>;
    }
    return <Suspense fallback={<p className="analytics-echart-loading" role="status">Preparing chart renderer…</p>}>
      {this.props.children}
    </Suspense>;
  }
}
...
const SKILLS_DEFAULT_WINDOW_MS = 30 * 24 * 60 * 60 * 1_000;

/** The selected page owns its work: tool dashboards and Skills stay quiet while hidden. */
export function AnalyticsPanel() {
  const [page, setPage] = useState<"dashboards" | "skills">("dashboards");
  return (
    <main className="analytics-shell">
      <nav className="analytics-page-nav" aria-label="Analytics pages">
        <button type="button" aria-current={page === "dashboards" ? "page" : undefined} onClick={() => setPage("dashboards")}>Dashboards</button>
        <button type="button" aria-current={page === "skills" ? "page" : undefined} onClick={() => setPage("skills")}>Skills</button>
      </nav>
      {page === "dashboards" ? <ToolAnalyticsPanel /> : <SkillsPanel />}
    </main>
  );
}
...
function SkillsPanel() {
  ...
  const [filters, setFilters] = useState<SkillQueryFilter>(() => ({ startMs: Date.now() - SKILLS_DEFAULT_WINDOW_MS, endMs: Date.now() }));
  const [result, setResult] = useState<SkillsQueryResponse | null>(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState<string | null>(null);
  const requestSequence = useRef(0);

  const load = useCallback(async (nextFilters: SkillQueryFilter) => {
    const sequence = ++requestSequence.current;
    setLoading(true);
    setError(null);
    try {
      const next = await rpc.call("skillsQuery", nextFilters);
      if (sequence === requestSequence.current) setResult(next);
    } catch (cause) {
      if (sequence === requestSequence.current) setError(cause instanceof Error ? cause.message : "Skills could not load retained evidence.");
    } finally {
  ...
  const changeFilters = useCallback((next: SkillsDashboardFilters) => {
    const request = skillsQueryFilterFromDashboard(next, filters);
    setFilters(request);
    void load(request);
  }, [filters, load]);

  const loadContributors = useCallback(async (ids: readonly string[]) => (
    (await rpc.call("skillsRawContributors", { filters, ids: [...ids] })).map(skillContributor)
  ), [filters, rpc]);

  if (result == null) {
    return loading
      ? <LoadingState title="Loading Skills…" detail="Reading the bounded retained skill snapshot." />
      : <EmptyState title="Skills could not load" detail={error ?? "No retained skill evidence is available."} action={() => void load(filters)} />;
  }
  return <SkillsDashboard
    data={skillsDashboardData(result, loading, error)}
...
function ToolAnalyticsPanel() {
  ...
  const loadDashboard = useCallback(async (
    ...
    try {
      ...
      const engine = await getBrowserAnalyticsEngineOnDemand();
  ...
  useEffect(() => {
    ...
    void getBrowserAnalyticsEngineOnDemand().catch(() => {
      ...
      {activeVisualization == null || activeQuery == null ? (
        ...
        <>
          ...
          {compilation.error != null ? (
            ...
            figure != null && (
              <>
                <EChartsChunkBoundary><EChartsFigure figure={figure} onIntent={openMenu} onController={(next) => { controller.current = next; }} /></EChartsChunkBoundary>
